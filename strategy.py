...
class FlowTracker:
    ...
    ALPHA      = 0.05   # how much 1 unit of net signed volume adjusts FV (tune on practice round)
...
class StockAStrategy:
    ...
    async def on_earnings(self, eps: float, tick: int) -> None:
        ...
        log(f"[A] *** EARNINGS *** EPS={eps} scale={EPS_SCALE_A} FV={old_fv}→{new_fv} "
            f"Δ={delta:+.1f} pos={self.client.positions['A']}")
...
class OptionsStrategy:
    ...
    async def _pcp_arb(self, strike: int) -> None:
        ...
        # Also check against implied B price for stronger confirmation
        s_impl = self.b_tracker.get_implied_price()

        if None in (c_bid, c_ask, p_bid, p_ask, s_bid, s_ask):
            return

        # Buy call, sell put, sell B  (edge = P_bid + S_bid - C_ask - K)
        ...
        # Sell call, buy put, buy B  (edge = C_bid - P_ask - S_ask + K)
    ...
    async def _box_arb(self, k1: int, k2: int) -> None:
        ...
        # Buy box:  long C1, short C2, short P1, long P2
        ...
        # Sell box:  short C1, long C2, long P1, short P2
    ...
    async def run(self) -> None:
        ...
        # Cancel unfilled option/B orders before re-checking
        for sym in ["B"] + [f"B_{t}_{k}" for t in ("C", "P") for k in (950, 1000, 1050)]:
